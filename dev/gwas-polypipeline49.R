...
args = c("in/config-test500-Kinship+PCs-noFilters.config")
...
runTasselGwas <- function (model) 
{
	...
	outTassel  = paste0("out/out-Tassel-", model)
	if (model=="Naive") {
		cmm=paste ("$TASSEL_HOME/run_pipeline.pl -fork1 -plink -ped", inGenoPED, "-map", inGenoMAP, 
				   "-fork2 -r", inPhenoTBL, "-combine3 -input1 -input2 -intersect -glm -export", outTassel)
		system (cmm)
	}else if (model=="KPCs") {
		cmm=sprintf ("scripts/tassel-pipeline-MLM-Kinship+PCs.sh %s %s %s %s",
					 inGenoPED, inGenoMAP, inPhenoTBL, outTassel)
		outFile =  list.files("out/", pattern="^(.*(Kinship).*(stats).*(txt)[^$]*)$", full.names=T)
		msg ()
		msg (model)
		msg (outFile)
		msg ()
	...
	outFile = paste0 (outTassel, "1.txt") 
	ts      = read.table (outFile, header=T)[,1:11]
	...
	outFile = paste0(outTassel,".scores") 
	write.table (file=outFile, tsArr, quote=F, sep="\t", row.names=F)
}
